...
AudioFile<double>::AudioBuffer resample(AudioFile<double>::AudioBuffer &input_file, double current_samplerate, double target_samplerate) {
    ...
    for (int channel_index = 0; channel_index < num_channels; channel_index++) {
        ...
        if (anti_alias) {
            for (int index = n_zero_crossing; index < (input_signal.size() - n_zero_crossing); index++) {
        ...
        for (int index = 0; index < output_buffer[channel_index].size(); index++) {
            ...
            assert(interpolation_factor >= 0.0 && interpolation_factor <= 1.0);
            ...
            for (int zero_cross_index = -n_zero_crossing; zero_cross_index <= n_zero_crossing; zero_cross_index++) {
                double sinc_value = sinc((double)zero_cross_index + interpolation_factor);
                signal_value = input_signal[input_buffer_index + zero_cross_index];
                double hann_value = hann_window(((double)zero_cross_index + (double)n_zero_crossing + interpolation_factor)/((double)interpolation_length - 1.0));
...
int main() {
    ...
    std::string filepath = "resampling_data/default_IR_441.wav";
    ...
    bool ok = output_file.save("resampling_data/output_file.wav", AudioFileFormat::Wave);
    assert(ok && "erreur dans l'enregistrement du fichier");
